...
namespace Coinbase.Net.UnitTests
{
    ...
    internal class SharedRestIntegrationTests
    {
        ...
        private ICoinbaseRestClientAdvancedTradeApiShared GetRestClient()
        {
            var collection = new ServiceCollection();
            collection.AddCoinbase(x => x.Rest.OutputOriginalData = true);
            ...
            return sp.GetRequiredService<ICoinbaseRestClient>().AdvancedTradeApi.SharedClient;
        }
        ...
        [Test]
        public async Task TestFuturesTradesHistoryRequests()
        {
            if (!ShouldRun())
                return;

            var client = GetRestClient();
            var result1 = await client.GetTradeHistoryAsync(new GetTradeHistoryRequest(_futuresSymbol, DateTime.UtcNow.AddDays(-1)));
            CheckResults([
                ("FuturesTrades", result1)
                ]);
        }
